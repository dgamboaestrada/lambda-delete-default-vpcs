...
def delete_igw(ec2, vpc_id):
  ...
  if igw:
    ...
    try:
      print("Detaching internet gateway: {}".format(igw_id))
      result = ec2.detach_internet_gateway(InternetGatewayId=igw_id, VpcId=vpc_id)
    except ClientError as e:
      print(e.response['Error']['Message'])

    try:
      print("Deleting internet gateway: {}".format(igw_id))
...
def delete_rtbs(ec2, args):
  ...
  if rtbs:
    for rtb in rtbs:
      ...
      try:
        print("Deleting route table: {}".format(rtb_id))
...
def delete_sgps(ec2, args):
  ...
  if sgps:
    for sgp in sgps:
      ...
      try:
        print("Deleting security group: {}".format(sg_id))
...
def handler(event, context):
  ...
  for region in regions:
    ...
    print('Cleaning region {}'.format(regions))
